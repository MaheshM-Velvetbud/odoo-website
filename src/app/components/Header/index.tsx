...
const Header = () => {
  ...
  return (
    <>
     <header
        className={`header left-0 top-0 z-40 flex w-full items-center h-20 ${sticky
                        ...
                        {menuItem.path ? (
                          <Link
                            ...
                            className={`flex py-2 text-lg  lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${usePathName === menuItem.path
                              ? "text-black "
                              : ` ${!sticky && usePathName.includes("/services/webdevelopment") ? "text-black" : "text-black"} text-gray-700 hover:text-primary hover:cursor-pointer `
                              }text-black`}
                          >
                            {menuItem.title}
                          </Link>
                          ...
                          <>
                            <p
                              ...
                              className={`${!sticky && usePathName.includes("/services/webdevelopment") ? "text-black" : " text-black"} flex cursor-pointer   items-center justify-between py-2 text-lg text-gray-700 group-hover:text-primary lg:mr-0 lg:inline-flex lg:px-0 lg:py-6`}
                            >
                              {menuItem.title}
                              <span className="pl-3">
                                ...
                                {menuItem.submenu?.slice(0, 8).map((submenuItem, index) => {
                                  return (
                                    <Link
                                      ...
                                      key={index}
                                      className="block rounded py-2.5 text-sm arrow-svg text-gray-700 hover:text-green-800 lg:px-3"
                                    >
                                      {submenuItem.title}
                                    </Link>
                                  );
                                ...
                                {menuItem.submenu?.slice(8).map((submenuItem, index) => {
                                  return (
                                    <Link
                                      href={submenuItem.path}
                                      key={index}
                                      className="block text-gray-700 rounded py-2.5 text-sm arrow-svg  hover:text-blue-800 lg:px-3"
                                    >
                                      {submenuItem.title}
                                    </Link>
                                  );
